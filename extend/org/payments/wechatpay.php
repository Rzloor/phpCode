...
class wechatpay implements Payment
{
    ...
    public function transfer($param){
        ...
        if(
            ...
            $result['status'] = true;                       //没有证书的时候，相当于客户没有配置证书，那么原路返回线上就直接做已退款操作就可以了，不实际去退了，具体的金额在支付后台做退款
            $result['msg'] = "微信支付证书没有上传，不能在线付款";
            return $result;
        }

        //取appid
        $appid_re = $this->getAppid('JSAPI_OFFICIAL');
        if(!$appid_re['status']){
            return $appid_re;
        }
        ...
        $data['mch_appid']        = $appid_re['data'];
        $data['mchid']            = $this->config['mch_id'];//商户号
        $data['nonce_str']        = self::getNonceStr();
        $data['partner_trade_no'] = $param['source_id'];//商户订单号
        $data['openid']           = $param['openid'];//用户openid
        $data['check_name']       = 'NO_CHECK';//NO_CHECK：不校验真实姓名 FORCE_CHECK：强校验真实姓名
        $data['re_user_name']     = '';//用户真实姓名 强校验真实姓名时必填
        $data['amount']           = $param['money']*100;//付款金额 单位为分
        $data['desc']             = $param['desc'];//备注
        $data['sign']             = $this->makeSign($data);
        ...
        $re = $this->fromXml($response);
        logs($re,true,'微信打款返回信息','transfer'.date('Ymd').'.html');
        if(!isset($re['return_code'])){
            $result['msg'] = '未知错误2';
            return $result;
        }
    ...
    //查询打款、转账信息
    public function getTransferInfo($source_id){
        $result = [
            'status' => false,
            'data' => 0,//打款状态:1-成功 2-失败 3-处理中 4-订单不存在
            'msg' => ''
        ];
        ...
        //查询企业付款
        $check_url = 'https://api.mch.weixin.qq.com/mmpaymkttransfers/gettransferinfo';

        $check_data['nonce_str']        = self::getNonceStr();
        $check_data['partner_trade_no'] = $source_id;
        $check_data['mch_id']           = $this->config['mch_id'];
        $check_data['appid']            = $appid_re['data'];
        $check_data['sign']             = $this->makeSign($check_data);

        $check_xml = $this->toXml($check_data);
        $check_response = $this->postXmlCurl($check_xml, $check_url, true, 6);

        if($check_response == ""){
            //出错了
            $result['msg'] = '查询错误：未知错误';
            return $result;
        }
        $check_re = $this->fromXml($check_response);
        logs($check_re,true,'微信打款查询返回信息','transfer'.date('Ymd').'.html');
        if(!isset($check_re['return_code'])){
            $result['msg'] = '查询错误：未知错误2';
            return $result;
        }

        if($check_re['return_code'] == 'SUCCESS'){
            if($check_re['result_code'] == 'SUCCESS'){
                if($check_re['status'] == 'SUCCESS'){
                    $result['msg'] = '转账成功';
                    $result['status'] = true;
                    $result['data'] = 1;
                }elseif($check_re['status'] == 'FAILED'){
                    $result['msg'] = '转账失败';
                    $result['status'] = true;
                    $result['data'] = 2;
                }elseif($check_re['status'] == 'PROCESSING'){
                    $result['msg'] = '处理中';
                    $result['status'] = true;
                    $result['data'] = 3;
                }
            }else{
                if($check_re['err_code'] == 'NOT_FOUND'){
                    $result['msg'] = '数据不存在';
                    $result['status'] = true;
                    $result['data'] = 4;
                }else{
                    $result['msg'] = $check_re['err_code'].'-'.$check_re['err_code_des'];
                    return $result;
                }
            }
        }else{
            $result['msg'] = $check_re['return_msg'];
            return $result;
        }
        return $result;
    }
