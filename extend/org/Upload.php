...
class Upload
{
     ...
     */
    public function upload($files = '',$image_storage)
    {
        if ('' === $files) {
            $files = $_FILES;
        }else{
        ...
        foreach ($files as $key => $file) {
            ...
            if (false === $subpath) {
                ...
                if ( $this->isLocal == false){
                    $file['savepath'] = $image_storage['bucket'] . $subpath;
                }else{
            ...
            $data = $this->uploadgo($file,$file['tmp_name']);
            $imgurl =$data;
            $info[$key] = $imgurl;
        }
        if (isset($finfo)) {
    ...
    private function check($file)
    {
        ...
        if (!$this->checkSize($file['size'])) {
            if ($this->getSize($file['size'])>5)
            $this->error = '上传文件最大为5M';
            return false;
        }
    ...
    public function uploadgo($file,$filename)
    {
        $ossClient = $this->Upload_Oss();
        if ($file) {
            $name = $file['name'];
            $tmp_name = $file['tmp_name'];
            $format = strrchr($name, '.');//截取文件后缀名如 (.jpg)
            /*判断图片格式*/
            $allow_type = ['.jpg', '.jpeg', '.gif', '.bmp', '.png', '.mp4'];
            ...
            try {
                if (getSetting('image_storage_params')) {
                    $config = array_merge(['type' => getSetting('image_storage_type')], getSetting('image_storage_params'));
                }
                $fileName = 'images/' .date("Ymd") . '/' .  hash("sha256", date('YmdHis', microtime()) . uniqid()). $format;
